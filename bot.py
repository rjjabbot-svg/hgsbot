...
async def fetch_products_from_notion() -> dict:
    ...
    try:
        ...
        for i, page in enumerate(data.get("results", [])):
            ...
            # Strains
            strains_raw = props.get("Strains", {}).get("rich_text", [])
            strains = strains_raw[0]["text"]["content"] if strains_raw else ""

            # Price tiers
            tiers_raw = props.get("Price Tiers", {}).get("rich_text", [])
            price_tiers = tiers_raw[0]["text"]["content"] if tiers_raw else ""
        ...
        return {
            "p1": {"name": "CBD Gummies 25mg (30ct)",  "price": 39.99, "strains": "", "price_tiers": ""},
            "p2": {"name": "CBD Tincture 1000mg",       "price": 54.99, "strains": "", "price_tiers": ""},
            "p3": {"name": "CBD Topical Cream 500mg",   "price": 34.99, "strains": "", "price_tiers": ""},
            "p4": {"name": "Hemp Flower 3.5g",          "price": 24.99, "strains": "", "price_tiers": ""},
            "p5": {"name": "Delta-8 Gummies (20ct)",    "price": 29.99, "strains": "", "price_tiers": ""},
            "p6": {"name": "CBD Capsules 25mg (60ct)",  "price": 44.99, "strains": "", "price_tiers": ""},
        }
...
async def handle_forwarded_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        ...
        await update.message.reply_text(
            ...
            f"💵 Prices: {tiers_preview or f'${parsed[\"lowest_price\"]:.2f}'}\n\n"
...
async def product_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build product detail message with strains and price tiers
    msg = f"*{product['name']}*\n"
