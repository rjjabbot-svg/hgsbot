...
# Global reference to the bot app for webhook use
_bot_app = None
    ...
    def do_POST(self):
        if self.path == '/order':
            try:
                ...
                if _bot_app:
                    threading.Thread(
                        target=lambda: asyncio.run(process_web_order(order)),
                        daemon=True
                    ).start()
...
async def process_web_order(order: dict):
    """Handle an order submitted from the Mini App website."""
    try:
        order_id    = next_order_id()
        placed_time = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
        crypto      = order.get("crypto", "BTC")
        total_usd   = float(order.get("total", 0))
        name        = order.get("name", "Unknown")
        address     = order.get("address", "")
        city        = order.get("city", "")
        state       = order.get("state", "")
        zip_code    = order.get("zip", "")
        tg_username = order.get("telegram_username", "Via Website")
        items       = order.get("items", [])
        items_text  = ", ".join(f"{i.get('size','')} {i.get('name','')}" for i in items)

        # Get live crypto price
        price = await get_crypto_price(crypto)
        if not price:
            price = 1.0
        crypto_amount = usd_to_crypto(total_usd, price)
        ...
        order_data = {
            ...
            "product": {"name": items_text, "price": total_usd},
        }
        await create_notion_order(order_data, order_id, total_usd, crypto_amount, placed_time)
        ...
        vendor_msg = (
            ...
            f"---\n"
            f"Crypto: {crypto_amount} {crypto} — PENDING PAYMENT\n"
            f"Wallet: {wallet}"
        )
        ...
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                "✅ Mark as Forwarded to Vendor",
                callback_data=f"forward_{order_id}_0"
            )
        ]])
        ...
        await _bot_app.bot.send_message(
            chat_id=ADMIN_CHAT_ID,
            text=(
                f"🌐 *NEW WEB ORDER — #{order_id}*\n\n"
                f"🕐 {placed_time}\n"
                f"💵 ${total_usd:.2f} via {crypto}\n\n"
                f"👤 {name}\n"
                f"📬 {address}, {city}, {state} {zip_code}\n"
                f"📦 {items_text}\n\n"
                f"*Vendor message 👇*\n\n"
                f"`{vendor_msg}`\n\n"
                f"Watch your {crypto} wallet — blockchain detection running!"
            ),
            parse_mode="Markdown",
            reply_markup=keyboard
        )

        # Start blockchain watcher (no customer chat_id for web orders — notify admin only)
        if crypto == "BTC":
            asyncio.create_task(
                wait_for_btc_payment(_bot_app, ADMIN_CHAT_ID, order_id, crypto_amount, total_usd, order_data)
            )
        else:
            asyncio.create_task(
                wait_for_sol_payment(_bot_app, ADMIN_CHAT_ID, order_id, crypto_amount, total_usd, order_data)
            )

    except Exception as e:
        logger.error(f"process_web_order error: {e}")

def keep_alive_pinger():
    """Ping ourselves every 10 minutes so Render never spins down."""
    import time
    while True:
        time.sleep(600)
        try:
